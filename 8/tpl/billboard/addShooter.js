...
if(href!==undefined){
    $('#box div.header div.admin span').html('<a href="billboard.html">榜单</a>>>射手榜编辑');
    var arr=href.split('&');
    ...
    $('input[name="totaltime"]').val(obj.totaltime);
    $('input[name="goal"]').val(obj.goal);

    //编辑榜单-积分榜
    $('#box button.submit').click(function(){
    	if($('div.show input[name="ranking"]').val()==''||$('div.show input[name="totaltime"]').val()==''||$('div.show input[name="goal"]').val()==''){
            ...
            $.ajax({
                url:'../../v1/manager/billboard/shooter/'+id,
                type:'put',
    ...
    $('#box button.submit').click(function(){
    	if($('div.show input[name="ranking"]').val()==''||$('div.show input[name="totaltime"]').val()==''||$('div.show input[name="goal"]').val()==''){
	        ...
	        obj = JSON.stringify(obj);
	        $.ajax({
	            url:'../../v1/manager/billboard/shooter',
	            type:'post',
	            contentType:'application/json',
	            data:obj,
	            success:function(data){
