...
if(href!==undefined){
    ...
    var type=obj.type;
    if(type==1){
        $('#box div.header div.admin span').html('<a href="billboard.html">榜单</a>>>主队积分榜编辑')
    }else{
        $('#box div.header div.admin span').html('<a href="billboard.html">榜单</a>>>客队积分榜编辑')
    }
    $.ajax({
        url:'../../v1/manager/billboard/teamscoreboard/init/'+id,
        type:'get',
        success:function(data){
            if(data.resultcode==1000){
    ...
    $('#box button.submit').click(function(){
        if($('div.show input[name="ranking"]').val()==''||$('div.show input[name="session"]').val()==''||$('div.show input[name="win"]').val()==''||$('div.show input[name="draw"]').val()==''||$('div.show input[name="loss"]').val()==''||$('div.show input[name="goal"]').val()==''||$('div.show input[name="lossgoal"]').val()==''||$('div.show input[name="integral"]').val()==''){
            ...
            $.ajax({
                url:'../../v1/manager/billboard/teamscoreboard/'+id,
                type:'put',
    ...
    $('#box button.submit').click(function(){
        if($('div.show input[name="ranking"]').val()==''||$('div.show input[name="session"]').val()==''||$('div.show input[name="win"]').val()==''||$('div.show input[name="draw"]').val()==''||$('div.show input[name="loss"]').val()==''||$('div.show input[name="goal"]').val()==''||$('div.show input[name="lossgoal"]').val()==''||$('div.show input[name="integral"]').val()==''){
            ...
            obj = JSON.stringify(obj);
            $.ajax({
                url:'../../v1/manager/billboard/teamscoreboard',
                type:'post',
                contentType:'application/json',
                data:obj,
                success:function(data){
