      ...
      obj.on('click', 'li', function(){
        ...
        if(l>n){
          ...
          $this.find('a').addClass(active);
			...
			//选中页
			checkpageFn();
			function checkpageFn() {
        var pageshow = parseInt(cheakPage);
        var nums = odevity(n);//判断奇偶
        if(l>n){
          if(pageshow > l-nums/2&&pageshow < l+1){
            //最后几项
          ...
          $('.' + active).removeClass(active);
          for (var i = 0 ; i < l; i ++) {
          	if (obj.find('ul li a').eq(i).html() == cheakPage) {
          		obj.find('ul li a').eq(i).addClass(active);
          	}
          }
        }
			}
